...
pub async fn get_article(state: State<'_, AppState>, id: String) -> Result<Article, String> {
    article_repo::get_by_id(&state.db, &id)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn search_articles(
    ...
    article_repo::search(&state.db, &q)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn create_article(
    ...
    article_repo::create(&state.db, cmd)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn update_article(
    ...
    article_repo::update(&state.db, cmd)
        .await
        .map_err(|e| e.to_string())
}
...
pub async fn delete_article(state: State<'_, AppState>, id: String) -> Result<(), String> {
    article_repo::delete(&state.db, &id)
        .await
        .map_err(|e| e.to_string())
}
