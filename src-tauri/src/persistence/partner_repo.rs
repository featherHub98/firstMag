use sqlx::SqlitePool;
use crate::domain::{Partner, CreatePartner, DomainResult, DomainError};
...
pub async fn list(pool: &SqlitePool, partner_type: Option<&str>) -> DomainResult<Vec<Partner>> {
    let rows = if let Some(pt) = partner_type {
        sqlx::query_as::<_, Partner>(
            "SELECT * FROM partners WHERE partner_type = ? ORDER BY name"
        )
        .bind(pt)
        .fetch_all(pool)
        .await?
...
pub async fn create(pool: &SqlitePool, cmd: CreatePartner) -> DomainResult<Partner> {
    ...
    sqlx::query(
        "INSERT INTO partners (id, partner_type, code, name, address, phone, email, tax_id, credit_limit, balance, notes, active, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, 1, ?, ?)"
    )
    .bind(&partner.id)
...
pub async fn search(pool: &SqlitePool, q: &str) -> DomainResult<Vec<Partner>> {
    ...
    let rows = sqlx::query_as::<_, Partner>(
        "SELECT * FROM partners WHERE name LIKE ?1 OR code LIKE ?1 OR phone LIKE ?1 ORDER BY name LIMIT 50"
    )
    .bind(&pattern)
    .fetch_all(pool)
    .await?;
